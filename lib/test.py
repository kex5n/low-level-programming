...
class Test:
    ...
    def perform(self, arg):
        ...
        (output, code) = res
        print('"', arg,'" ->',  res)
        if type(arg) == str and not self.name in ("parse_uint", "parse_int"):
            arg = arg.encode("ascii")
        # if type(output) == str:
        #     output = output.encode("ascii")
        try:
            check_res = self.checker( arg, output, code )
        except:
            breakpoint()
        if not check_res:
            breakpoint()
...
tests=[ 
        ...
        xor rdi, rdi
        syscall""", 
        lambda i, o, r: int(o) == unsigned_reinterpret(int(i))),
        ...
        mov rax, 60
        syscall""", 
        lambda i, o, r: (i == "" and r == 0 ) or i[0] == r ),
